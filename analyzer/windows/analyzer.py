...
def upload_files(folder):
    ...
    try:
        if os.path.exists(log_folder):
            ...
            log.warning(f'Folder at path "{log_folder}" does not exist, skip')
            return
    except IOError as e:
        log.warning(f'Unable to access folder at path "{log_folder}": {e}')
        return
    ...
    for root, dirs, files in os.walk(log_folder):
        for file in files:
            ...
            try:
                ...
                log.error(f'Unable to upload file at path "{file_path}": {e}')
...
class Analyzer:
    ...
    def complete(self):
        ...
        for pid in INJECT_LIST:
            log.warning("Monitor injection attempted but failed for process {pid}")
    ...
    def run(self):
        ...
        log.debug(f"Starting analyzer from: {os.getcwd()}")
        log.debug(f"Storing results at: {PATHS['root']}")
        log.debug(f"Pipe server name: {PIPE}")
        log.debug(f"Python path: {os.path.dirname(sys.executable)}")
        ...
        if not self.config.package:
            ...
            log.info(f'Automatically selected analysis package "{package}"')
        # Otherwise just select the specified package.
        else:
            package = self.config.package
            log.info(f'Analysis package "{package}" has been specified')
        # Generate the package path.
        package_name = f"modules.packages.{package}"
        # Try to import the analysis package.
        try:
            log.debug(f'Importing analysis package "{package}"...')
            __import__(package_name, globals(), locals(), ["dummy"])
            # log.debug(f'Imported analysis package "{package}"')
        ...
        log.debug(f'Initializing analysis package "{package}"...')
        self.package = package_class(self.options, self.config)
        # log.debug(f'Initialized analysis package "{package}"')
        ...
        if self.package.options.get("dll") is not None:
            ...
            log.info(f"Analyzer: DLL set to {MONITOR_DLL} from package {package_name}")
        else:
            log.info(f"Analyzer: Package {package_name} does not specify a DLL option")
        ...
        if self.package.options.get("dll_64") is not None:
            ...
            log.info(f"Analyzer: DLL_64 set to {MONITOR_DLL_64} from package {package_name}")
        else:
            log.info(f"Analyzer: Package {package_name} does not specify a DLL_64 option")
        ...
        if self.package.options.get("loader") is not None:
            ...
            log.info(f"Analyzer: Loader set to {LOADER32} from package {package_name}")
        else:
            log.info(f"Analyzer: Package {package_name} does not specify a loader option")
        ...
        if self.package.options.get("loader_64") is not None:
            ...
            log.info(f"Analyzer: Loader_64 set to {LOADER64} from package {package_name}")
        else:
            log.info(f"Analyzer: Package {package_name} does not specify a loader_64 option")
        ...
        for loader, name, ispkg in pkgutil.iter_modules(auxiliary.__path__, prefix):
            ...
            try:
                log.debug(f'Importing auxiliary module "{name}"...')
                __import__(name, globals(), locals(), ["dummy"])
                # log.debug(f'Imported auxiliary module "{name}"')
            except ImportError as e:
                log.warning(f'Unable to import the auxiliary module "{name}": {e}')
        ...
        for module in Auxiliary.__subclasses__():
            ...
            #    continue
            try:
                log.debug(f'Initializing auxiliary module "{module.__name__}"...')
                aux = module(self.options, self.config)
                # log.debug(f'Initialized auxiliary module "{module.__name__}"')
                aux_avail.append(aux)
                # log.debug(f'Trying to start auxiliary module "{module.__name__}"...')
                aux.start()
            except (NotImplementedError, AttributeError):
                log.warning(f"Auxiliary module {module.__name__} was not implemented")
            except Exception as e:
                log.warning(f"Cannot execute auxiliary module {module.__name__}: {e}")
            else:
                log.debug(f"Started auxiliary module {module.__name__}")
        ...
        while self.do_run:
            ...
            try:
                ...
                # throw a warning.
                    ...
                    log.warning(f'The package "{package_name}" check function raised an exception: {e}')
            finally:
                # Zzz.
                KERNEL32.Sleep(1000)
            ...
            for pid in self.process_list.pids:
                ...
                if proc.is_alive():
                    try:
                        ...
                        log.error(f"Unable to set terminate event for process {proc.pid}")
                        continue
                    log.info(f"Terminate event set for process {proc.pid}")
                if self.config.terminate_processes:
                    ...
                    if proc.is_alive() and pid not in self.CRITICAL_PROCESS_LIST and not proc.is_critical():
                        log.info(f"Terminating process {proc.pid} before shutdown")
                        proc_counter = 0
                        while proc.is_alive():
                            if proc_counter > 5:
                                try:
                                    ...
                                    continue
                            log.info(f"Waiting for process {proc.pid} to exit")
                            KERNEL32.Sleep(1000)
                            proc_counter += 1
        ...
        try:
            ...
            log.warning(f'The package "{package_name}" finish function raised an exception: {e}')
        ...
        try:
            ...
            log.warning(f'The package "{package_name}" package_files function raised an exception: {e}')
        ...
        for aux in aux_avail:
            try:
                ...
                log.warning(f"Exception running finish callback of auxiliary module {aux.__class__.__name__}: {e}")
...
class Files(object):
    ...
    def add_pid(self, filepath, pid, verbose=True):
        ...
        if pid not in self.files[filepath.lower()]:
            ...
            if verbose:
                log.info(f"Added pid {pid} for {filepath}")
    ...
    def add_file(self, filepath, pid=None, category="files", metadata=""):
        ...
        if filepath.lower() not in self.files:
            log.info(f"Added new file to list with pid {pid} and path {filepath}")
    ...
    def dump_file(self, filepath, metadata="", pids=False, ppids=False, category="files"):
        ...
        if not os.path.isfile(filepath):
            log.warning(f"File at path {filepath} does not exist, skipping")
            return False
        ...
        try:
            ...
            log.info(f'Error dumping file from path "{filepath}": {e}')
            return
        ...
        try:
            ...
            log.error(f'Unable to upload dropped file at path "{filepath}": {e}')
        except Exception as e:
            log.error(e, exc_info=True)
...
class CommandPipeHandler(object):
    ...
    def _handle_loaded(self, data):
        ...
        log.info(f"Loaded monitor into process with pid {pid}")
    ...
    # same as below but we don't want to inject any DLLs because
    ...
    def _handle_kprocess(self, data):
        ...
        if process_id and process_id not in (self.analyzer.pid, self.analyzer.ppid, self.analyzer.process_list.pids):
            ...
            if not in_protected_path(filename):
                ...
                log.info(f"Announce process name : {filename}")
        self.analyzer.process_lock.release()
    ...
    def _handle_ksubvert(self, data):
        for pid in self.analyzer.process_list.pids:
            log.info(f"Process with pid {pid} has terminated")
    ...
    def _handle_service(self, servname):
        if not ANALYSIS_TIMED_OUT:
            ...
            log.info(f'Announced starting service "{servname}"')
            if not self.analyzer.MONITORED_SERVICES:
                ...
                if self.analyzer.SERVICES_PID:
                    ...
                    log.error(f"Unable to monitor service {servname}")
    ...
    def _handle_kill(self, data):
        ...
        process_id = int(data)
        log.info(f"Process with pid {process_id} has terminated")
    ...
    def _inject_process(self, process_id, thread_id, mode):
        ...
        if not self.analyzer.files.is_protected_filename(filename):
            ...
            log.info(f"Injected into process with pid {proc.pid} and name {filename}")
            ...
            if process_id not in (self.analyzer.pid, self.analyzer.ppid):
                ...
                if process_id not in self.analyzer.process_list.pids:
                    ...
                    if self.analyzer.SERVICES_PID and process_id == self.analyzer.SERVICES_PID:
                        self.analyzer.CRITICAL_PROCESS_LIST.append(int(self.analyzer.SERVICES_PID))
                    log.info(f"Announced {'64-bit' if is_64bit else '32-bit'} process name: {filename} pid: {process_id}")
                ...
                log.warning(f"Received request to inject process with pid {process_id}, skipped")
    ...
    def _handle_file_dump(self, file_path):
        ...
        if b"\\CAPE\\" in file_path:
            ...
            if os.path.exists(file_path):
                ...
                log.info(f"File doesn't exist, {file_path}")

    def _handle_dumpreqs(self, data):
        if not data.isdigit():
            log.warning(f"Received DUMPREQS command with an incorrect argument {data}")
            return
        pid = int(data)
        if pid not in self.tracked:
            log.warning(f"Received DUMPREQS command but there are no reqs for pid {pid}")
            return

        dumpreqs = self.tracked[pid].get("dumpreq", [])
        for addr, length in dumpreqs:
            log.debug(f"Tracked dump req ({pid}, {addr}, {length})")
    ...
    def dispatch(self, data):
        ...
        if not data or b":" not in data:
            ...
            if not fn:
                log.critical(f"Unknown command received from the monitor: {data.strip()}")
            else:
                try:
                    response = fn(arguments)
                except Exception as e:
                    log.error(e, exc_info=True)
                    log.exception(f"Pipe command handler exception occurred (command {command} args {arguments})")
...
if __name__ == "__main__":
    ...
    # When user set wrong package, Example: Emotet package when submit doc, package only is for EXE!
        ...
        try:
            urlopen("http://127.0.0.1:8000/status", urlencode(data).encode())
        except Exception as e:
            print(e)
        sys.exit()
