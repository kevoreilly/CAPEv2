...
def generate_graphs(args_dict):
    ...
    # # Try and import the graphs (shifted)
    try:
        graphs = get_graph_modules()
        log.info("Got graph modules")
...
def main():
    ...
    parser.add_argument(
        "--json", action="store_true", default=__json__, help="Output graphs as json with graph images encoded as Base64"
    )
