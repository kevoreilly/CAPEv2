    ...
    for c in control:
        if c and c.startswith("tcp://"):
            ...
            if tmp:
                ...
                if len(tmp) == 2:
                    remcos_config_list.append({"Version": remcos_config.get("version", ""), "C2": tmp[0], "Port": tmp[1], "Password": ""})
                elif len(tmp) == 3 and tmp[2] != "":
                    # we can include the passprhase
                    remcos_config_list.append({"Version": remcos_config.get("version", ""), "C2": tmp[0], "Port": tmp[1], "Password": tmp[2]})
                else:
