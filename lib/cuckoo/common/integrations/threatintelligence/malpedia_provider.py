...
class MalpediaProvider(FamilyProvider):
    ...
    def __init__(self, options):
        ...
        self.host = (self.options.get("host") or DEFAULT_HOST).strip().rstrip("/")
        self.apitoken = (self.options.get("api_key") or "").strip() or None
        self.max_references = int(self.options.get("max_references", 4) or 0)
        self._session = None
    ...
    def _harvest_ids(cls, obj) -> List[str]:
        """Collect Malpedia family ids from a loosely-typed blob.

        Iterative (explicit stack) rather than recursive: the blob comes from
        an external API, so a pathologically deep response must not be able to
        raise RecursionError. Children are pushed in reverse so that pop()
        visits them in document order -- _best_candidate() returns the FIRST
        acceptable id, so ordering is behaviourally significant.
        """
        found = []
        stack = [obj]
        while stack:
