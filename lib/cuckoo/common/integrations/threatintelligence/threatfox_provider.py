...
class ThreatFoxProvider(IndicatorProvider):
    ...
    def __init__(self, options):
        ...
        self.host = (self.options.get("host") or DEFAULT_HOST).strip()
        self.apikey = (self.options.get("api_key") or "").strip() or None
        self.exact_match = _as_bool(self.options.get("exact_match", False))
        ...
        self.retries = int(self.options.get("retries", 3) or 0)
        self.backoff = float(self.options.get("backoff", 0.5) or 0.0)
        self._session = None
    ...
    def available(self) -> bool:
        ...
        if not self.apikey:
            log.warning(
                "ThreatFox enabled but api_key (Auth-Key) is empty in the [threatfox] "
                "section of threat_intel.conf. abuse.ch rejects unauthenticated "
                "requests; get a free key at https://auth.abuse.ch/ . Provider skipped."
            )
            return False
        return True
    ...
    def _post(self, payload):
        ...
        while True:
            ...
            if status in ("illegal_auth_key", "unauthorized", "missing_auth_key"):
                log.warning("ThreatFox auth rejected (query_status=%s); check [threatfox] api_key.", status)
                return None
