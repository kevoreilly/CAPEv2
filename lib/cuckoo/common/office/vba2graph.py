...
lst_autorun = [
    ...
    u"\w+_Painted",
    u"\w+_Change",
    u"\w+_DocumentBeforePrint",
    u"\w+_DocumentOpen",
    u"\w+_DocumentBeforeClose",
    u"\w+_DocumentBeforeSave",
    u"\w+_GotFocus",
    u"\w+_LostFocus",
    u"\w+_MouseHover",
    u"\w+_Resize",
    ...
    u"\w+_Layout",
    u"\w+_Painting",
    u"\w+_BeforeNavigate2",
    u"\w+_BeforeScriptExecute",
    u"\w+_DocumentComplete",
    u"\w+_DownloadBegin",
    u"\w+_DownloadComplete",
    u"\w+_FileDownload",
    u"\w+_NavigateComplete2",
    u"\w+_NavigateError",
    u"\w+_ProgressChange",
    u"\w+_PropertyChange",
    u"\w+_PropertyChange",
    u"\w+_StatusTextChange",
    u"\w+_TitleChange",
    u"\w+_MouseMove",
    u"\w+_MouseEnter",
    u"\w+_MouseLeave",
...
lst_mal_case_sensetive += ["\.caption", "\.text", "\.value", "\.ControlTipText", "\.tag", "\.CustomDocumentProperties", "\.AlternativeText"]
    ...
    for func_name in vba_func_dict:
        ...
        for dic_key in dict_items:
            ...
            DG.nodes[func_name]["keywords"] = (
                DG.nodes[func_name]["keywords"] + "<font color='" + keyword_color + "'>" + dic_key + "[" + str(keyword_count) + "]" + "</font>"
            )
