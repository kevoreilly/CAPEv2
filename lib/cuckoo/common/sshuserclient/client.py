...
logger = logging.getLogger("sshuserclient")
...
class AcceptNewPolicy(MissingHostKeyPolicy):
    def missing_host_key(self, client, hostname, key):
        logger.warning(
            (f"No host key for {hostname} found in known_hosts, " "accepting & adding to host keys file"),
        )
...
class SSHClient(ParamikoClient):
    ...
    def connect(
        ...
        **kwargs,
        ...
        (hostname, config, forward_agent, missing_host_key_policy, host_keys_file,) = self.parse_config(
            hostname,
            kwargs,
    ...
    def parse_config(
        ...
        if "proxycommand" in host_config:
            ...
            for i, hop in enumerate(hops):
                ...
                c = SSHClient()
                c.connect(hop_hostname, _pyinfra_ssh_config_file=ssh_config_file, sock=sock, **hop_config)
