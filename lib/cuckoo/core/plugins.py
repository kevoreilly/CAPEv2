...
class RunStatistics:
    ...
    This class cout some fileds for need.
    """
    ...
    def getDroppedFileCount(self):
        """Count DroppedFiles.
        @return int value.
        """
        return len(self.results.get("dropped"))

    def getRuningProcessesCount(self):
        """Count running processes.
        @return int value.
        """
        if self.results.get("behavior").get("processtree"):
            return len(self.results.get("behavior").get("processtree"))

    def getApiCallsAndRegistryCount(self):
        ...
        try:
            ...
            log.exception('Failed to RunStatistics "%s" :%s', self.__class__.__name__, e)

    def getDomainsCount(self):
        """Count Domains
        @return int value.
        """
        return len(self.results.get("network").get("domains"))

    def getSignaturesAndAlertCount(self):
        """Count signature, signature alert, crash issues, anti issues.
        @return four int values.
        """
        try:
            signature_count = len(self.results.get("signatures"))
    ...
    def getFilesWrittenCount(self):
        """Count fileswirtten.
        @return int value.
        """
        return len(self.results.get("behavior").get("summary").get("write_files"))
    ...
    def run(self):
        ...
        detail["dropped_files"] = self.getDroppedFileCount()
        detail["running_processes"] = self.getRuningProcessesCount()
        detail["api_calls"],detail["registry_keys_modified"] = self.getApiCallsAndRegistryCount()
        detail["domains"] = self.getDomainsCount()
        detail["signatures_total"],detail["signatures_alert"],\
        detail["crash_issues"],detail["anti_issues"] = self.getSignaturesAndAlertCount()
        detail["files_written"] = self.getFilesWrittenCount()
        if db.add_statistics_to_task(self.task["id"], **detail):
            log.info("Statistics sucessed!")
        else:
            log.info("Statistics failed!")
