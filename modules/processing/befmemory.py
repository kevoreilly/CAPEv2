...
class VolatilityManager:
    ...
    def __init__(self, memfile):
        ...
        conf_path = os.path.join(CUCKOO_ROOT, "conf", "befmemory.conf")
        if not os.path.exists(conf_path):
            log.error("Configuration file befmemory.conf not found")
            self.voptions = False
            return

        self.voptions = Config("befmemory")
...
class BefMemory(Processing):
    ...
    def run(self):
        ...
        self.befmemory_path = os.path.join(CUCKOO_ROOT, "storage", "analyses", str(self.task["id"]), "befmemory.dmp")
        if os.path.exists(self.befmemory_path):
            try:
                vol = VolatilityManager(self.befmemory_path)
                results = vol.run()
            except Exception:
                log.exception("Generic error executing volatility")
                if self.voptions.basic.delete_memdump_on_exception:
                    try:
                        os.remove(self.befmemory_path)
                    except OSError:
                        log.error('Unable to delete memory dump file at path "%s"', self.befmemory_path)
        else:
            log.error("Memory dump not found: to run volatility you have to enable befmemory_dump")
