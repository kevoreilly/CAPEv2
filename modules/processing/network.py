...
class Pcap2:
    ...
    def run(self):
        ...
        pcap_exists = path_exists(self.pcap_path)
        log.info("DEBUG: path_exists(%s) = %s", self.pcap_path, pcap_exists)
        if not pcap_exists:
...
class NetworkAnalysis(Processing):
    ...
    def run(self):
        ...
        if HAVE_TSHARK and not using_mixed_pcap:
            try:
                ...
                log.exception("Error running tshark-based PCAP analysis")
