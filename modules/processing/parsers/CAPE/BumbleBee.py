...
def extract_config_data(data, pe, config_match):
    ...
    try:
        ...
        campaign_id_offset = pe.get_offset_from_rva(campaign_id_rva + int.from_bytes(config_match.group('campaign_id'), byteorder='little'))
    ...
    try:
        ...
        botnet_id_rva = pe.get_rva_from_offset(config_match.start() + int(len(config_match.group('campaign_id_ins'))) + int(len(config_match.group('botnet_id_ins'))))
        botnet_id_offset = pe.get_offset_from_rva(botnet_id_rva + int.from_bytes(config_match.group('botnet_id'), byteorder='little'))
    ...
    # Get C2 ciphertext
    try:
        c2s_rva = pe.get_rva_from_offset(config_match.start() + int(len(config_match.group('campaign_id_ins'))) + int(len(config_match.group('botnet_id_ins'))) + int(len(config_match.group('c2s_ins'))))
...
def extract_config(data):
    ...
    regex = re.compile(rb'(?<campaign_id_ins>\x48\x8D.(?P<campaign_id>....))(?P<botnet_id_ins>\x48\x8D.(?P<botnet_id>....))(?P<c2s_ins>\x48\x8D.(?P<c2s>....))', re.DOTALL)
    # Extract Key
