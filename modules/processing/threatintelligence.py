...
class ThreatIntelligence(Processing):
    ...
    def run(self):
        ...
        try:
            ...
            log.warning("Could not read threat_intel.conf: %s", err)
            return {}
        # Global behaviour; each engine additionally gets its own section
        # overlaid on top of these (see TIConfig.provider_options).
        options = config.globals

        indicator_providers = get_enabled_indicator_providers(config)
        family_providers = get_enabled_family_providers(config)
        ...
        actor_providers = get_enabled_actor_providers(config) if actor_enabled else []
        ...
        families = []
        if family_providers:
            from_threatfox = _as_bool(config.section("malpedia").get("from_threatfox", True))
