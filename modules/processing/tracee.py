...
sec_events = ["sched_process_exec", "stdio_over_socket","k8s_api_connection","aslr_inspection","proc_mem_code_injection","docker_abuse","scheduled_task_mod","ld_preload","cgroup_notify_on_release","default_loader_mod","sudoers_modification","sched_debug_recon","system_request_key_mod","cgroup_release_agent","rcd_modification","core_pattern_modification","proc_kcore_read","proc_mem_access","hidden_file_created","anti_debugging","ptrace_code_injection","process_vm_write_inject","disk_mount","dynamic_code_loading","fileless_execution","illegitimate_shell","kernel_module_loading","k8s_cert_theft","proc_fops_hooking","syscall_hooking","dropped_executable"]
...
class TraceeAnalysis(Processing):
    ...
    def run(self):
        ...
        subprocess.run(r'''grep -v '\\"processName\\":\\"strace\\"' ''' + logpath + ' > ' + logpath + ".cleaner", shell=True, capture_output=True, text=True)
        ...
        while ln:
            ...
            try:
                ...
                log.info(f"Could not process Tracee line: {ln}")
                continue
                ...
                if lg["syscall"] == "execve":
                    for arg in lg["args"]:
                        if arg["name"] == "argv":
                            ...
                            tree.get_child(lg["parentProcessId"]).add_child(lg["processId"], {
                                "desc": arg["value"], "cmdline": arg["value"], #"full": lg,
                                "env": arg2,
                            })
