...
class MaecReport(Report):
    ...
    def create_process_tree_node(self, process, process_children, is_root):
        ...
        process_obj = self.package['observable_objects'][
            self.pidObjectMap[str(process['process_id'])]]
        ...
        if str(process['process_id']) in process_children:
            process_obj['child_refs'] = [self.pidObjectMap[x]
                                         for x in
                                         process_children[str(process['process_id'])]]
        ...
        for process in processes:
            if 'parent_id' in process and str(process['parent_id']) in process_pids:
                if str(process['parent_id']) not in process_children:
                    process_children[str(process['parent_id'])] = [str(process
                        ['process_id'])]
                else:
                    process_children[str(process['parent_id'])].append(str(process
                        ['process_id']))
