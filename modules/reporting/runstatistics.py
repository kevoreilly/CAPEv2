...
class RunStatistics(Report):
    ...
    order = 10001  # used in the reporting module and required here.
    ...
    def run(self, results):
        ...
        detail["crash_issues"],detail["anti_issues"] = self.getSignaturesAndAlertCount(results)
        detail["files_written"] = self.getFilesWrittenCount(results)
        if main_db.add_statistics_to_task(task_id, **detail):
            log.info("Run statistics sucessed!")
        else:
            log.info("Run statistics failed!")
