...
class ClamAV(Signature):
    ...
    def run(self):
        ...
        if self.results.get("target", {}).get("category", "") not in ("url", "pcap"):
            if self.results.get("target", {}).get("file", {}).get("clamav") and self.results.get("target", {}).get("file", {}).get(
                "sha256"
            ):
                for detection in self.results.get("target", {})["file"]["clamav"]:
            ...
            if entry.get("file_info", {}).get("clamav") and entry.get("file_info", {}).get("sha256"):
                for detection in entry["file_info"]["clamav"]:
                    ...
                    lentry = "{}, suricata_extracted_files, src:{}, sp:{}, dst:{}, dp:{}".format(
                        detection, entry.get("srcip", ""), entry.get("sp", ""), entry.get("dstip", ""), entry.get("dp", "")
                    )
