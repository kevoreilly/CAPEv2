...
class RIG_JS(Signature):
    ...
    def on_call(self, call, process):
        ...
        str6 = [
            "TjNOTlhRWGlXUFdOV2VYaVhpVzNOTllNWGhYaF",
            "4zTk5YUVhpV1BXTldlWGlYaVczTk5ZTVhoWGhY",
            "OM05OWFFYaVdQV05XZVhpWGlXM05OWU1YaFhoW",
        ]
        if (
            "VBscript" in buf
            and "String.fromCharCode" in buf
            and "window.execScript" in buf
            and any(e in buf for e in str1)
            and any(e in buf for e in str2)
        ...
        if (
            "if('getBoxObjectFor'" in buf
            and "mozInnerScreenX" in buf
            and "isFirefox++"
            and "if('WebKitCSSMatrix'" in buf
            and "webkitStorageInfo" in buf
            and "isChrome++" in buf
            and "='sandbox'" in buf
            and "createElement('iframe')" in buf
            and "history.pushState" in buf
            and "documentElement.webkitRequestFullScreen" in buf
            and "isIE++" in buf
