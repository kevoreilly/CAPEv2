...
class TestPcap2Tshark(unittest.TestCase):
    ...
    def test_pcap2_http_parsing(self):
        # Setup mocks using patch.dict to replace functions in the network module
        import modules.processing.network as network_module

        print("Setting up mocks with patch.dict...")
        ...
        with patch.dict(network_module.__dict__, {
            'path_exists': mock_exists,
            'path_mkdir': mock_mkdir,
            'path_delete': mock_delete,
            'path_write_file': mock_write_file,
