        ...
        while True:
            ...
            if tasks is not None:
                for task in tasks:
                    ...
                    if hasattr(reporting_conf, "backscatter") and reporting_conf.backscatter.enabled:
                        try:
                            ...
                            if files:
                                try:
                                    r = requests.post(reporting_conf.backscatter.url,
                                        params={
                                            "callback_uri": reporting_conf.backscatter.callback_url+ "/"+str(task.main_task_id)+"/",
                                    ...
                                    log.info("Can't report to backscatter")
                                except Exception as e:
                                    log.error(e)
                        except Exception as e:
                            log.error(e)
    ...
    def fetch_latest_reports(self):
        ...
        while not self.stop_dist.isSet():
            ...
            try:
                ...
                t = db.query(Task).filter_by(node_id=node_id, task_id=task["id"], retrieved=False, finished=False).order_by(Task.id.desc()).first()
                if t is None:
...
def cron_cleaner():
    ...
    if tasks is not None:
        for task in tasks:
            node = nodes[task.node_id] #db.query(Node).filter_by(id = task.node_id).first()
            if node:
                try:
