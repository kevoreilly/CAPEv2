...
def main():
    ...
    try:
        from lib.cuckoo.common.integrations.threatintelligence.config import load_config
        from lib.cuckoo.common.integrations.threatintelligence.registry import all_provider_names
        config = load_config(all_provider_names())
        options = config.globals
    except Exception as err:
        print(f"[FAIL] Could not read threat_intel.conf: {err}")
        return
    if not options:
        print("[FAIL] [threatintelligence] section empty or missing in threat_intel.conf.")
        return

    def _show(key, value):
        if key.endswith(("_api", "_key", "_token", "_secret")) and value:
            value = "<set>"
        print(f"         {key} = {value!r}")

    print("[ OK ] threat_intel.conf read. Global [threatintelligence]:")
    for key in sorted(options):
        _show(key, options[key])
    for name in all_provider_names():
        section = config.section(name)
        if not section:
            print(f"       [{name}] section absent (engine treated as disabled)")
            continue
        print(f"       [{name}] enabled={config.is_enabled(name)}")
        for key in sorted(section):
            if key != "enabled":
                _show(key, section[key])
    ...
    ind = get_enabled_indicator_providers(config)
    fam = get_enabled_family_providers(config)
    for p in ind:
